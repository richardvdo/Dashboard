...
function cal ()
{
  ...
  cal_timeout = setTimeout("cal()", 14400000);
}
...
function email ()
{
  $.ajax({
   ...
   //   horloge_content = "tssssuc" ;

 // $("#email").html(html);
  
  email_timeout = setTimeout("email()", 60000);
}
...
function puissance()
{
  $.ajax({
    ...
    type: "GET",
    url: "./ajax.php",
    data: "block=puissance",
    success: function(html){
      $('#puissance').load('teleinfo/teleinfo_graph_v3_inc.php')
    }
  });

  puissance_timeout = setTimeout("puissance()", 180000);
...
// var url_eth1_down = "pict/triton_ts3.png";
// var url_eth1_up = "pict/triton_ts3.png";
  // var img_oberon_down = $("<img />").attr("src", url_eth1_down);
  // $("#img_oberon_down").attr("src", url_eth1_down);

   // var img_eth0_up = $("<img />").attr("src", url_eth1_up);
   // $("#img_eth0_up").attr("src", url_eth1_up);
 ...
 $.ajax({
  ...
  // })
    ...
    // });
	 ...
	 $.getJSON('./ajax.php?block=ifstat', function (data) {
  ...
  ifstat_timeout = setTimeout("ifstat()", 500000);
}
